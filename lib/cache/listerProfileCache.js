...
let incompleteToastShown = false;
...
// Returns true and flips the flag on first call; false on every call after,
// for the life of the page (or until clearListerProfileCache runs). The
// page-load toast uses this so it only fires once per session, independent
// of the AccountSettings tab toast, which fires on every mount.
export function shouldShowIncompleteToast() {
  if (incompleteToastShown) return false;
  incompleteToastShown = true;
  return true;
}

// Call on logout so the next login re-fetches and can show the toast again.
export function clearListerProfileCache() {
  cachedProfile = undefined;
  inFlightRequest = null;
  incompleteToastShown = false;
}
