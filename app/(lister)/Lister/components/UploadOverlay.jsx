// app/(lister)/Lister/components/UploadOverlay.jsx
"use client";
...
export default function UploadOverlay({
  ...
  if (stage === "Uploading") {
    ...
    const imagesDone =
      images && stats.imagesDoneCount >= stats.imagesTotalCount;
    const videoNotDone =
      haveVideo && stats.videoDoneCount < stats.videoTotalCount;
    activeIndex = images
      ? imagesDone && videoNotDone
        ? 1
        : 0
      : videoNotDone
        ? 1
  ...
  const isPreparing = stage === "Preparing upload";
  const isByteStep =
    !isPreparing && (activeStep.key === "images" || activeStep.key === "video");
  ...
  const imagesPercent =
    stats.imagesTotal > 0
      ? Math.min(
          100,
          Math.round((stats.imagesLoaded / stats.imagesTotal) * 100),
        )
      : 0;
  const videoPercent =
    stats.videoTotal > 0
  ...
  const activePercent = isPreparing
    ? Math.min(100, uploadProgress?.percent ?? 0)
    : isByteStep
      ? activeStep.key === "images"
        ? imagesPercent
        : videoPercent
      : null;
  ...
  const overallPercent = Math.round(
    (STEPS.reduce((sum, step, i) => {
      ...
      if (isPreparing) return sum + activePercent / 100;
      ...
      <div className={styles.body}>
        <div className={styles.headingRow}>
          <div>
            ...
            <p className={styles.subtitle}>
              Hang on there as we put your listing to the list of winning
              listers.
            </p>
          </div>
        ...
        <div className={styles.footerRow}>
          <div className={styles.currentStatusGroup}>
            ...
            {isUploadingImages && (
              <span
                className={`${styles.imageCountInline} ${styles.imageUploadPulsing}`}>
