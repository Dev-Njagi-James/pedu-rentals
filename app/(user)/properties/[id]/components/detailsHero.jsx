...
export default function PropertyDetails({ listing }) {
  ...
  const mediaItems = (media ?? [])
    ...
    .map((m) => {
      ...
      return {
        image_url: m.cloudinary_url ?? m.image_url,
        video_url: null,
        position: m.position,
      };
    });
  ...
  return (
    <div className={styles.page}>
      ...
      <div
        ref={infoRef}
        className={`${styles.infoRow} ${infoVisible ? styles.visible : ""}`}>
        <div className={styles.infoLeft}>
          ...
          <div className={styles.priceRow}>
            ...
            <span className={styles.price}>
              KSH {Number(property_price).toLocaleString("en-KE")}
            </span>
          </div>
          ...
          {(Number(review_count) > 0 || Number(avg_rating) > 0) && (
            <div className={styles.ratingRow}>
              ...
              <span className={styles.ratingText}>
                {Number(avg_rating) ?? 0} ({Number(review_count) ?? 0}{" "}
        ...
        <div className={styles.agentCard}>
          ...
          <div className={styles.agencyRow}>
            ...
            <span className={styles.agencyName}>
              {agentInfo.organisationName ?? "Agency name"}
            </span>
          </div>
        </div>
      </div>
            ...
            {reviews.map((r) => (
              <div key={r.review_id} className={styles.reviewCard}>
                <div className={styles.reviewHeader}>
                  ...
                  <span className={styles.reviewHandle}>
                    @{r.username ?? "user"}
                  </span>
                </div>
                {r.review_text && (
